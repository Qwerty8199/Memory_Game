...
public class Board extends JPanel implements ActionListener {
 ...
 private static final String DEFAULT_IMAGE_FILENAME_PREFIX = "img-";
  String DEFAULT_IMAGE_FOLDER = "/images/animes/";
  String HIDDEN_IMAGE_PATH ;
  String EMPTY_IMAGE_PATH = "/images/lego/img-25.jpg";
 ...
 public Board(String img,int max) {
  super();

  DEFAULT_IMAGE_FOLDER = img;
  MAX_NUM_OF_CARDS = max;
  ConfigureBoard(max);
  
  HIDDEN_IMAGE_PATH = img
		   + DEFAULT_IMAGE_FILENAME_PREFIX + "26"
		   + DEFAULT_IMAGE_FILENAME_SUFFIX;
  
		  
  
  System.out.println(DEFAULT_IMAGE_FOLDER);
  System.out.println(MAX_NUM_OF_CARDS);
  System.out.println(NUMBER_OF_ROWS);
  System.out.println(NUMBER_OF_COLUMNS);
  System.out.println(NUMBER_OF_PAIRS);
...
public void actionPerformed(ActionEvent e) {
    ...
    float tempScore = (( MAX_NUM_OF_CARDS ) / ( numOfFailedAttempts )) * 100;
    int urScore = (int) (tempScore);

     if(urScore > _high) {
    	 name = JOptionPane.showInputDialog( "Solved!! \n enter ur name");
     }
     else {
    	 name = JOptionPane.showInputDialog( " Woah!! HIGH SCORE \n enter ur name"+" "+urScore);
     }
     
    
     
     JOptionPane.showMessageDialog(null,
       " Your results:\n"+name+"\n  score = "+urScore,"RESULTS",
       JOptionPane.INFORMATION_MESSAGE); 
     send.insert(urScore, name);
    }// if solved
 ...
 private void ConfigureBoard(int max) {
	 
	  get_high = new datahand();
	  
	  _high = get_high.return_high();
	  
	  System.out.println("high score : "+_high);
